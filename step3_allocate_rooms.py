    ...
    try:
        ...
        for col in ['Morning', 'Evening']:
            timetable_df[col] = timetable_df[col].apply(
                lambda x: [] if pd.isna(x) else [s.strip() for s in str(x).split(';') if s.strip() and s.strip().upper() != 'NO EXAM']
            )
        ...
        if 'Roll' in roll_name_df.columns:
            roll_name_df.rename(columns={'Roll': 'rollno'}, inplace=True)
        if 'Name' in roll_name_df.columns:
            roll_name_df.rename(columns={'Name': 'name'}, inplace=True)
        ...
        return timetable_df, course_roll_df, roll_name_dict, room_df
    except Exception as e:
        logging.error(f"Error reading input files: {str(e)}", exc_info=True)
        raise
...
def format_room_excel(filepath, date_str, session, room, course, df):
    ...
    wb.save(filepath)
    print(f"Formatted and saved: {filepath}")
...
def main():
    ...
    try:
        timetable_df, course_roll_df, roll_name_dict, room_df = read_input_files()
        ...
        for _, row in timetable_df.iterrows():
            ...
            for session in ['Morning', 'Evening']:
                ...
                if check_clashes(subjects, course_roll_df):
                    ...
                    continue
                allocation, unallocated, room_capacity = allocate_subjects(
                    subjects, course_roll_df, room_df, buffer, arrangement_type
                )
                generate_outputs(date_str, day_str, session, allocation, roll_name_dict, room_df, room_capacity)
